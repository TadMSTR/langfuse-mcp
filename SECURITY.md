...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/langfuse-mcp/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[langfuse-mcp] Security Report`.
...
- SSRF via the `LANGFUSE_HOST` configuration
- Injection in trace, session, or score query parameters passed to the Langfuse API
- Information disclosure through trace or session data containing sensitive LLM inputs or outputs
- Dependency vulnerabilities with a plausible exploitation path in langfuse-mcp's usage
